<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            ...
            <flux:text class="mt-1">{{ $mode === 'me' ? 'Your own verb project, at a glance.' : 'How each kid is doing, at a glance.' }}</flux:text>
        </div>
        <flux:radio.group wire:model.live="kid" variant="segmented">
            @foreach ($kids as $k)
                <flux:radio :value="(string) $k->id" :label="$k->name" />
            @endforeach
            <flux:radio value="me" label="Me" />
        </flux:radio.group>
    </div>
    ...
    @if ($mode === 'me')
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            <flux:card class="py-4 text-center">
                <div class="text-3xl font-semibold {{ $myDueToday > 0 ? 'text-amber-600 dark:text-amber-400' : '' }}">{{ $myDueToday }}</div>
                <div class="mt-1 text-xs text-zinc-500">due today</div>
            </flux:card>
            <flux:card class="py-4 text-center">
                <div class="text-3xl font-semibold text-sky-600 dark:text-sky-400">{{ $myNew }}</div>
                <div class="mt-1 text-xs text-zinc-500">new in queue</div>
            </flux:card>
            <flux:card class="py-4 text-center">
                <div class="text-3xl font-semibold">{{ $myTraining }}</div>
                <div class="mt-1 text-xs text-zinc-500">in training</div>
            </flux:card>
            <flux:card class="py-4 text-center">
                <div class="text-3xl font-semibold text-green-600 dark:text-green-400">{{ $myKnown }}</div>
                <div class="mt-1 text-xs text-zinc-500">verbs known</div>
            </flux:card>
            <flux:card class="py-4 text-center">
                <div class="text-3xl font-semibold">{{ $myWaiting }}</div>
                <div class="mt-1 text-xs text-zinc-500">waiting</div>
            </flux:card>
            <flux:card class="py-4 text-center">
                <div class="text-3xl font-semibold">{{ $myAccuracy === null ? '—' : $myAccuracy.'%' }}</div>
        ...
        <flux:card class="space-y-3">
            <div class="flex items-center justify-between">
                <flux:heading size="lg">Verb catalog</flux:heading>
                <flux:text class="text-zinc-500">
                    {{ $myKnown }} known · {{ $myTraining }} in training · {{ $myTotal }} verbs
                    @if ($myReviewedToday > 0) · {{ $myReviewedToday }} reviewed today @endif
                </flux:text>
            </div>
            <div class="flex h-3 overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-700" role="img"
                aria-label="{{ $myKnown }} of {{ $myTotal }} verbs known, {{ $myTraining }} in training">
                @if ($myTotal > 0)
                    <div class="h-full bg-green-500" style="width: {{ round($myKnown / $myTotal * 100) }}%"></div>
                    <div class="h-full bg-sky-500/70" style="width: {{ round($myTraining / $myTotal * 100) }}%"></div>
                @endif
            </div>
            <div class="flex gap-4 text-xs text-zinc-500">
                <span class="flex items-center gap-1.5"><span class="inline-block size-2 rounded-full bg-green-500"></span> known ({{ $myKnownPct }}%)</span>
                <span class="flex items-center gap-1.5"><span class="inline-block size-2 rounded-full bg-sky-500/70"></span> in training</span>
                <span class="flex items-center gap-1.5"><span class="inline-block size-2 rounded-full bg-zinc-300 dark:bg-zinc-600"></span> waiting</span>
            </div>
        </flux:card>
        ...
        <flux:card class="space-y-3">
            <div class="flex items-center justify-between">
                ...
                <flux:button :href="route('admin.practice')" wire:navigate size="sm" icon="play" variant="primary">Practice</flux:button>
            </div>
            <div class="flex h-28 items-end gap-[2px]" role="img" aria-label="Reviews due per day over the next 14 days">
                @foreach ($myUpcoming as $day)
                    <div class="group flex min-w-0 flex-1 flex-col items-center justify-end gap-1"
                        ...
                        @if ($day['count'] > 0)
                            ...
                            <div class="w-full max-w-4 rounded-t-[4px] {{ $loop->first ? 'bg-amber-500' : 'bg-indigo-500' }}"
                                style="height: {{ max(6, round($day['count'] / $myUpcomingMax * 76)) }}px"></div>
                        @else
        ...
        {{-- Trouble spots --}}
        <flux:card class="space-y-3">
            <flux:heading size="lg">Trickiest verbs</flux:heading>
            @forelse ($myTrouble as $row)
                <div class="flex items-center justify-between gap-3 border-b border-zinc-100 pb-2 last:border-0 last:pb-0 dark:border-zinc-800">
                    ...
                    <flux:badge color="amber" size="sm" class="shrink-0">{{ $row['lapses'] }} miss{{ $row['lapses'] === 1 ? '' : 'es' }} / {{ $row['reps'] }} passes</flux:badge>
                </div>
            @empty
